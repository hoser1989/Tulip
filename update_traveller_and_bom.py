...
def sendToTulip(df, table_id):
    url = f'https://hiab-test.tulip.co/api/v3/tables/{table_id}/records'
...
def updateTulipSyncRecord(record_id, fieldId):
    url_base = 'https://hiab-test.tulip.co/api/v3/tables/F8msta7LuWpSHqXPz/records'
    user = 'apikey.2_YbECmsfBSjhGwYf3T'
    pwd = 'GO01NhVXEikyXQ-uksiQ4v6nPplEtoAW-sVklVtUAfs'
    ...
    with requests.Session() as session:
        ...
        url = f"{url_base}/{record_id}"  # Tworzenie URL dla PUT
...
def createTulipSyncRecord(record_id):
    url = 'https://hiab-test.tulip.co/api/v3/tables/F8msta7LuWpSHqXPz/records'
    user = 'apikey.2_YbECmsfBSjhGwYf3T'
    pwd = 'GO01NhVXEikyXQ-uksiQ4v6nPplEtoAW-sVklVtUAfs'
...
def checkIfTulipOrderExists(order_number):
    user = 'apikey.2_YbECmsfBSjhGwYf3T'
    pwd = 'GO01NhVXEikyXQ-uksiQ4v6nPplEtoAW-sVklVtUAfs'
    url = 'https://hiab-test.tulip.co/api/v3/tables/Jii944sA7s3kS5pu8_DEFAULT/records?filters=[{"field":"id","arg":"' + order_number + '","functionType":"equal"}]&limit=1'
...
def checkTulipIfSynced(order_number):
    user = 'apikey.2_YbECmsfBSjhGwYf3T'
    pwd = 'GO01NhVXEikyXQ-uksiQ4v6nPplEtoAW-sVklVtUAfs'
    url = 'https://hiab-test.tulip.co/api/v3/tables/F8msta7LuWpSHqXPz/records?filters=[{"field":"id","arg":"' + order_number + '","functionType":"equal"}]&limit=1'
...
def getTulipProductionOrders(datetime_from, datetime_to, all = False):
    ...
    if all == False:
        url = f'https://hiab-test.tulip.co/api/v3/tables/Jii944sA7s3kS5pu8_DEFAULT/records?filters={filters_str}'
    else:
        url = f'https://hiab-test.tulip.co/api/v3/tables/Jii944sA7s3kS5pu8_DEFAULT/records'
...
def productionSpecificationSummary(order_number):
    ...
    con = oracledb.connect(user="ecq", password="ecq105", dsn=or_dns)
    cursor = con.cursor()
...
#Execute
result = getTulipProductionOrders('2024-11-25T00:00:00Z','2024-11-26T00:00:00Z')
for index,row in result.iterrows():
    ...
    if sync_status.empty:
        ...
        updateTulipSyncRecord(row.id, 'jrbjn_traveller')
        # update bom
        bom = billOfMaterial(row.id)
        sendToTulip(bom, 'K6y2f8AiFpK8SbscT')
        updateTulipSyncRecord(row.id, 'uazuq_bom')
    else:
